...
export default function DemoPage({ searchParams }: { searchParams: PageSearchParams }) {
  ...
  const copy = {
    ru: {
      ...
      subtitle:
        "Готовый маршрут для показа покупателю, отелю или техническому ревьюеру: от первого запроса гостя до оплаты, кабинета, in-stay сервисов и менеджерской панели.",
      ...
      scenarios: [
        ...
        },
        {
          ...
          text: "Откройте каталог, выберите объект и покажите room-type availability: свободные номера, окна дат, цену и CTA на оформление.",
          link: catalogHref,
          cta: "Открыть каталог",
        },
        {
        ...
        },
        {
          ...
          text: "Покажите партнерский кабинет: объекты, номера, availability, брони, рестораны, room service и операционные статусы.",
          link: loginHref,
          cta: "Партнерский вход",
        },
        {
          icon: "shield" as const,
          title: "Коммерческий и legal слой",
          text: "Покажите страницу для отелей, privacy, terms, refund policy и контакты. Это закрывает базовый due diligence перед продажей.",
          link: hotelHref,
          cta: "Для отелей",
        },
      ],
      script: [
        ["0-2 мин", "Откройте страницу для отелей и объясните value: direct booking, AI concierge, manager workspace."],
        ["2-4 мин", "Откройте гостевое демо: поиск Dubai, карточки объектов, AI-консьерж и быстрые фильтры."],
        ["4-6 мин", "Выберите объект, покажите свободные номера, тарифы, cancellation logic и переход в checkout."],
        ["6-8 мин", "Проведите demo payment, откройте guest account и покажите in-stay сервисы."],
        ["8-10 мин", "Откройте partner login/manager workspace и покажите операционный контроль для отеля."],
      ],
